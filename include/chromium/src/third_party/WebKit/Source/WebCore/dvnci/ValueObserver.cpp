...
namespace WebCore {
    ...
    public:
        ...
        DVNExpressionEventImpl(const AtomicString& eventType, const dvnci::short_value& val, PassRefPtr<EventTarget> target) :
        DVNExpressionEvent(eventType, val, target), value_(val) {
        }
    ...
    PassRefPtr<DVNExpressionEvent> DVNExpressionEvent::create(const AtomicString& eventType, const dvnci::short_value& val, PassRefPtr<EventTarget> target) {
        return adoptRef(new DVNExpressionEventImpl(eventType, val, target));
    }

    DVNExpressionEvent::DVNExpressionEvent(const AtomicString& eventType, const dvnci::short_value& val, PassRefPtr<EventTarget> target) :
    Event(eventType, false, false) {
        setTarget(target);
    }
