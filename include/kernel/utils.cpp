...
namespace dvnci {
    ...
    string    strcomma_to_dot(string val) {
        boost::replace_all(val, ",", ".");
        return val;}

    string    strdot_to_comma(string val) {
        boost::replace_all(val, ".", ",");
        return val;}
    ...
    string&   clear_first_null_digit(string& vl) {
        if (vl.empty()) return vl;
        string::size_type it = vl.find_first_not_of("0");
        vl = (it != string::npos) ? vl.substr(it) : "0";
        return vl;}

    string&  fill_first_null_digit(string& vl, size_t needcsize) {
        if (vl.size() >= needcsize) return vl;
    ...
    std::string num64_and_type_cast(const num64& val, tagtype type, const std::string& formt){
        ...
        num64* val_ = const_cast<num64*> (&val);
        switch (type) {
            case TYPE_NODEF:  {boost::format format(formt.empty() ? "%8.2f" : formt.c_str()); format % (*reinterpret_cast<double*> (val_)); return format.str();}
            case TYPE_DOUBLE:  {boost::format format(formt.empty() ? "%8.2f" : formt.c_str()); format % (*reinterpret_cast<double*> (val_)); return format.str();}
            case TYPE_FLOAT: {boost::format format(formt.empty() ? "%8.2f" : formt.c_str()); format % (*reinterpret_cast<float*> (val_)); return format.str();}
            case TYPE_DISCRET: return val != 0 ? "1": "0";
            case TYPE_NUM64: {boost::format format(formt.empty() ? "%d" : formt.c_str()); format % (*reinterpret_cast<num64*> (val_)); return format.str();}
            case TYPE_UNUM64: {boost::format format(formt.empty() ? "%u" : formt.c_str()); format % (*reinterpret_cast<unum64*> (val_)); return format.str();}
            case TYPE_NUM32: {boost::format format(formt.empty() ? "%d" : formt.c_str()); format % (*reinterpret_cast<num32*> (val_)); return format.str();}
            case TYPE_UNUM32: {boost::format format(formt.empty() ? "%u" : formt.c_str()); format % (*reinterpret_cast<unum32*> (val_)); return format.str();}
            case TYPE_NUM16: {boost::format format(formt.empty() ? "%d" : formt.c_str()); format % (*reinterpret_cast<num16*> (val_)); return format.str();}
   ...
   std::string num64_and_type_cast(const num64& val, tagtype type, onum ladsk){
    ...
    void statistic_functor::operator()(const dt_val_pair& vl){
            switch (type){
                    case REPORT_STATISTIC_MIN:{ if (vl.second==vl.second) {value = (value!=value) ? vl.second : (value>vl.second ? vl.second : value); cnt++;} return;}
                    case REPORT_STATISTIC_MAX: { if (vl.second==vl.second) {value = (value!=value) ? vl.second : (value<vl.second ? vl.second : value); cnt++;} return;}
                    default: { if (vl.second==vl.second) {value = (value!=value) ? vl.second : vl.second + value; cnt++;} return;}}}
