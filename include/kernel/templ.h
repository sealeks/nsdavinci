...
public:
    ...
    virtual ~intrusive_sync_share_ptr_tmpl() {
        if (!(sh_ptr.use_count() - 1)) {
            DEBUG_STR_DVNCI(INTUSIVE_DESTRUCTOR);
        };
    };
...
public:
    ...
    virtual ~callable_shared_ptr() {
        if (!(sh_ptr.use_count() - 1)) {
            DEBUG_STR_DVNCI(CALLABDLE__DESTRUCTOR);
        }
    };
...
public:
    ...
    virtual ~intrusivesync_callable_shared_ptr() {
        if (!(sh_ptr.use_count() - 1)) {
            DEBUG_STR_DVNCI(INTRUSIVE_SYNC_CALLABDLE__DESTRUCTOR);
        };
    };
