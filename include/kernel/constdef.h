...
#define THD_TRY_LOCK(a)  boost::mutex::scoped_try_lock( a )
...
#define INP_UPGRATE_LOCK(a)    inp_lock_upgrate iplock( a );
    ...
    const std::string EXCL_CORRECT_ENTETYNAME_REGEXTAMPL = "^num$|^abs$|^real$|^bool$|^min$|^max$|^sin$|^cos$|^tan$|^sinh$|^cosh$|^tanh$|^pow$|^sqrt$|^sqr$|^nan$|^round$|^seil$|^format$"
            "|^asin$|^acos$|^atan$|^log10$|^log$|^now$|^exp$|^ln$|^rnd$|^e$|^pi$|^null$|^user$|^access$|^floor$|^trunc$|^ack$|^nack$|^alarm$|^tags$|^alarmlevel$";
