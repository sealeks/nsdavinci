...
namespace dvnci {
    namespace custom {
        namespace net {
         ...
         ns_error localnetintf::auth_req( const std::string& user, const std::string& user){
             return 0;}
     ...
     ns_error localnetintf::req_data_item( const vect_num64& sids, vect_data_item& lines , vect_data_item_str& linesstr, vect_error_info& errors){
          errors.clear();
          lines.clear();
          linesstr.clear();
          for (vect_num64::const_iterator it=sids.begin(); it!=sids.end(); ++it){
    ...
    ns_error localnetintf::req_data_item(vect_data_item& lines , vect_data_item_str& linesstr, vect_error_info& errors){
          errors.clear();
          lines.clear();
          linesstr.clear();
    ...
    ns_error localnetintf::req_send_commands(const vect_command_data& cmds, vect_error_info& errors){
          errors.clear();
    ...
    ns_error localnetintf::req_report(const vect_reporttask& tasks ,vect_report_value_data& dt, vect_error_info& errors){
        return 0;}
           
    
    
    ns_error localnetintf::req_event(const vect_eventtask& tasks ,vect_event_value_item& dt, vect_error_info& errors){
        return 0;}
    
    
    ns_error localnetintf::req_trend(const vect_trendtask& tasks,vect_report_value_data& dt , vect_error_info& errors){
        return 0;}
    
    
    ns_error localnetintf::req_alarms(const unum64& giud, vect_alarms_data& dt){
        return 0;}
    
    
    ns_error localnetintf::req_journal(const unum64& guid, const unum64& cursor, const unum64& cnt,vect_journal_data& dt){
        return 0;}    
