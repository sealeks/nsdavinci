        ...
        namespace net {
            ...
            inline static num64 packdata(vlvtype vld, tagtype tp,  ns_error err, bool al=false,  bool kv=false) {
                return  (((0xF & static_cast<num64> ((al ? 0x1 : 0x0) | (kv ? 0x2 : 0x0)) ) << 48) & 0xF000000000000LL) |
                        (((0xFF & static_cast<num64> (err) ) << 32) & 0xFFFF00000000LL) |
                        (((0xFF & static_cast<num64> (vld) ) << 16) & 0xFFFF0000LL) | 
                        (0xFFFF & tp);}

            inline static vlvtype unpackvalid(num64 v ) {
                return static_cast<vlvtype> (((v & 0xFF0000LL) >> 32) & 0xFF);}
            ...
            protected:
                ...
                ns_error assign_resp_items(const resp_add_items& resp, vect_sid_key& sids,  vect_error_item& errors);
                void assign_req_items(req_add_items_ex& req, const vect_cid_key_ex& cids);
                ns_error assign_resp_items(const resp_add_items_ex& resp, vect_sid_key_ex& sids,  vect_error_ex_item& errors);
                void assign_req_remitems(req_remove_items& req, const vect_num64& cids);
                ns_error assign_resp_remitems(const resp_remove_items& resp, vect_num64& succ_sids , vect_error_item& errors);
                void assign_req_values(req_data_item& req);
                void assign_req_values(req_data_item1& req, const vect_num64& sids);
                ns_error assign_resp_values(const resp_data_item& resp, vect_data_item& lines , vect_data_item_str& linesstr, vect_error_item& errors);
                void assign_req_values(req_data_item_ex& req);
                void assign_req_values(req_data_item_ex1& req, const vect_num64& sids);
                ns_error assign_resp_values(const resp_data_item_ex& resp, vect_data_item& lines , vect_data_item_str& linesstr, vect_sid_key_ex& sids, vect_error_item& errors);
                void assign_req_commands(req_send_commands& req, const vect_command_data& cmds);
                ns_error assign_resp_commands(const resp_send_commands& resp, vect_error_item& errors);
                void assign_req_reports(req_reporttask& req, const vect_reporttask& tasks);
                ns_error assign_resp_reports(const resp_reporttask& resp, vect_report_value_data& dt, vect_error_item& errors);
                void assign_req_events(req_eventtask& req, const vect_eventtask& tasks);
                ns_error assign_resp_events(const resp_eventtask& resp, vect_event_value_item& dt, vect_error_item& errors);
                void assign_req_trends(req_trendtask& req, const vect_trendtask& tasks);
                ns_error assign_resp_trends(const resp_trendtask& resp, vect_trend_value_data& dt , vect_error_item& errors);
                void assign_read_alarms(req_alarms& req, const unum64& giud);
                ns_error assign_resp_alarms(const resp_alarms& resp, vect_alarms_data& dt);
                void assign_read_journal(req_journal& req, const unum64& guid, const unum64& cursor, const unum64& cnt);
                ns_error assign_resp_journal(const resp_journal& resp, vect_journal_data& dt);
