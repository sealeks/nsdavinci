...
namespace database {
            ...
            bool db_task_executor::execute_report_task() {
                ...
                for (str_set::const_iterator key = task->tags().begin(); key != task->tags().end(); ++key) {
                    ...
                    if (it == trenddef.end()) {
                        ...
                        tmp.error = (dbdriver->select_report(it->second.cod, it->second.type, task->start(), task->stop(), *datamap)) ? SUCCESS : NOINDEX;
                        tmp.cod = it->second.cod;
                        tmp.comment = it->second.comment;
                        tmp.eu = it->second.eu;
                        tmp.mineu = it->second.mineu;
            ...
            bool db_task_executor::execute_select_task() {
                select_db_task_ptr task;{
                    ...
                    ns_error error = (dbdriver->select(task->req(), *result)) ? SUCCESS : NOINDEX;
            ...
            bool db_task_executor::execute_journal_task() {
                select_db_journal_task_ptr task;{
                    ...
                    ns_error error = (dbdriver->select_journal(task->start(), task->stop(), task->filter(),  *result)) ? SUCCESS : NOINDEX;
            ...
            bool db_task_executor::execute_debug_task() {
                select_db_debug_task_ptr task;{
                    ...
                    ns_error error = (dbdriver->select_debug(task->start(), task->stop(), task->filter(),  *result)) ? SUCCESS : NOINDEX;
