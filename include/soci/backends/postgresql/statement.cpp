...
int generate_pg_error(bool rslt, const char* errcd, const char* msg){
        if (!rslt){
    ...
    statement_type stType)
{
    ...
    if (stType == st_repeatable_query)
    {
        ...
        if (res == NULL)
        {
            generate_pg_error(0,0,0);
        }
        ExecStatusType status = PQresultStatus(res);
        if (status != PGRES_COMMAND_OK)
        {
            //throw soci_error(PQresultErrorMessage(res));
            generate_pg_error(res, PQresultErrorField(res, PG_DIAG_SQLSTATE), PQresultErrorMessage(res));
        }
        PQclear(res);
    }
            ...
            for (int i = 0; i != numberOfExecutions; ++i)
            {
                ...
                if (numberOfExecutions > 1)
                {
                    ...
                    if (status != PGRES_COMMAND_OK)
                    {
                        ...
                        generate_pg_error(result_,PQresultErrorField(result_, PG_DIAG_SQLSTATE), PQresultErrorMessage(result_));
                    }
                    PQclear(result_);
                }
            }
