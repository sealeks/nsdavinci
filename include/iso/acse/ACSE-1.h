...
const boost::array<boost::asio::asn::oidindx_type, 5 >  ACSE_1_OID_ARR = { 2  ,  2  ,  0  ,  1  ,  1 };
...
namespace ACSE_1  {
	...
	//It is  INTERNAL SEQUENCE  
		...
		struct  Context_list_sequence_of{
			...
			Presentation_context_identifier  pci;
			Abstract_syntax_name  abstract_syntax;
			transfer_syntaxes_type  transfer_syntaxes;
...
//It is   CHOICE
	...
	enum Authentication_value_enum {
		...
		Authentication_value_charstring_type , 
		Authentication_value_bitstring_type , 
		Authentication_value_external_type , 
		Authentication_value_other_type , 
	};
	...
	//It is  INTERNAL SEQUENCE  
		...
		struct  other_type{
			...
			MECHANISM_NAME  other_mechanism_name;
			any_type  other_mechanism_value;
	...
	//end==============================================================
		...
		BOOST_ASN_VALUE_CHOICE(charstring, graphicstring_type,  Authentication_value_charstring_type)
		BOOST_ASN_VALUE_CHOICE(bitstring, bitstring_type,  Authentication_value_bitstring_type)
		BOOST_ASN_VALUE_CHOICE(external, external_type,  Authentication_value_external_type)
		BOOST_ASN_VALUE_CHOICE(other, other_type,  Authentication_value_other_type)
		 ...
		 template<typename Archive> void serialize(Archive& arch){
			...
			if (arch.__input__()){
				...
				switch(arch.test_class()){
					...
					case 0x80: {
						switch(__tag_id__){
							case 0: { if(BOOST_ASN_IMPLICIT_TAG(value<graphicstring_type > (true , Authentication_value_charstring_type) ,0)) return; else free(); break;}
							case 1: { if(BOOST_ASN_IMPLICIT_TAG(value<bitstring_type > (true , Authentication_value_bitstring_type) ,1)) return; else free(); break;}
							case 2: { if(BOOST_ASN_IMPLICIT_TAG(value<external_type > (true , Authentication_value_external_type) ,2)) return; else free(); break;}
							case 3: { if(BOOST_ASN_IMPLICIT_TAG(value<other_type > (true , Authentication_value_other_type) ,3)) return; else free(); break;}
						default:{}}
						;}
					default:{
						;}}}
				 ...
				 switch(type()){
					case Authentication_value_charstring_type: {BOOST_ASN_IMPLICIT_TAG(value<graphicstring_type > (false , Authentication_value_charstring_type) ,0); break;}
					case Authentication_value_bitstring_type: {BOOST_ASN_IMPLICIT_TAG(value<bitstring_type > (false , Authentication_value_bitstring_type) ,1); break;}
					case Authentication_value_external_type: {BOOST_ASN_IMPLICIT_TAG(value<external_type > (false , Authentication_value_external_type) ,2); break;}
					case Authentication_value_other_type: {BOOST_ASN_IMPLICIT_TAG(value<other_type > (false , Authentication_value_other_type) ,3); break;}
					default:{}}}
		}
	};
...
//It is   SEQUENCE  
	...
	struct  presentation_data_values_type : public BOOST_ASN_CHOICE_STRUCT(presentation_data_values_type_enum) {
			...
			BOOST_ASN_VALUE_CHOICE(simple_ASN1_type, any_type,  presentation_data_values_type_ABSTRACT_SYNTAX)
...
//It is   CHOICE
	...
	struct  User_Data : public BOOST_ASN_CHOICE_STRUCT(User_Data_enum) {
		...
		BOOST_ASN_VALUE_CHOICE(fully_encoded_data, PDV_list,  User_Data_PDV_list)
		 ...
		 template<typename Archive> void serialize(Archive& arch){
			...
			if (arch.__input__()){
				...
				switch(arch.test_class()){
					...
					case 0x80: {
						switch(__tag_id__){
							case 0: { if(BOOST_ASN_EXPLICIT_TAG(value<PDV_list > (true , User_Data_PDV_list) ,0)) return; else free(); break;}
						default:{}}
						;}
					default:{
				 ...
				 switch(type()){
					...
					case User_Data_PDV_list: {BOOST_ASN_EXPLICIT_TAG(value<PDV_list > (false , User_Data_PDV_list) ,0); break;}
					default:{}}}
		}
	};
...
//It is   CHOICE
	...
	enum Associate_source_diagnostic_enum {
		...
		Associate_source_diagnostic_acse_service_user_type , 
		Associate_source_diagnostic_acse_service_provider_type , 
	};
	...
	struct  Associate_source_diagnostic : public BOOST_ASN_CHOICE_STRUCT(Associate_source_diagnostic_enum) {
		...
		BOOST_ASN_VALUE_CHOICE(acse_service_user, int,  Associate_source_diagnostic_acse_service_user_type)
		BOOST_ASN_VALUE_CHOICE(acse_service_provider, int,  Associate_source_diagnostic_acse_service_provider_type)
		 ...
		 template<typename Archive> void serialize(Archive& arch){
			...
			if (arch.__input__()){
				...
				switch(arch.test_class()){
					...
					case 0x80: {
						switch(__tag_id__){
							case 1: { if(BOOST_ASN_EXPLICIT_TAG(value<int > (true , Associate_source_diagnostic_acse_service_user_type) ,1)) return; else free(); break;}
							case 2: { if(BOOST_ASN_EXPLICIT_TAG(value<int > (true , Associate_source_diagnostic_acse_service_provider_type) ,2)) return; else free(); break;}
						default:{}}
						;}
					default:{
						;}}}
				 ...
				 switch(type()){
					case Associate_source_diagnostic_acse_service_user_type: {BOOST_ASN_EXPLICIT_TAG(value<int > (false , Associate_source_diagnostic_acse_service_user_type) ,1); break;}
					case Associate_source_diagnostic_acse_service_provider_type: {BOOST_ASN_EXPLICIT_TAG(value<int > (false , Associate_source_diagnostic_acse_service_provider_type) ,2); break;}
					default:{}}}
		}
	};
...
//It is   CHOICE
	...
	enum Syntactic_context_list_enum {
		...
		Syntactic_context_list_Default_Context_List , 
	};
	...
	struct  Syntactic_context_list : public BOOST_ASN_CHOICE_STRUCT(Syntactic_context_list_enum) {
		...
		BOOST_ASN_VALUE_CHOICE(default_contact_list, Default_Context_List,  Syntactic_context_list_Default_Context_List)
				...
				switch(arch.test_class()){
					...
					case 0x80: {
						switch(__tag_id__){
							...
							case 1: { if(BOOST_ASN_EXPLICIT_TAG(value<Default_Context_List > (true , Syntactic_context_list_Default_Context_List) ,1)) return; else free(); break;}
						default:{}}
						;}
					default:{
						;}}}
				 ...
				 switch(type()){
					...
					case Syntactic_context_list_Default_Context_List: {BOOST_ASN_EXPLICIT_TAG(value<Default_Context_List > (false , Syntactic_context_list_Default_Context_List) ,1); break;}
					default:{}}}
		}
	};
	...
	struct  ACRP_apdu__impl{
		...
		ACRP_apdu__impl()  : aso_qualifier() , aSO_context_name() , p_context_result_list() , user_information()  {}
		 ...
		 template<typename Archive> void serialize(Archive& arch){
			...
			BOOST_ASN_CHOICE(aso_qualifier);
...
//It is   SEQUENCE  
	...
	struct  ACRQ_apdu__impl{
		...
		BOOST_ASN_VALUE_FUNC_DECLARATE(ASO_context_name_list ,  aSO_context_name_list)
 		...
 		Syntactic_context_list  p_context_definition_list;
		boost::shared_ptr<User_information > user_information;   //  OPTIONAL
		BOOST_ASN_VALUE_FUNC_DECLARATE(User_information ,  user_information)
		...
		ACRQ_apdu__impl()  : aso_qualifier() , aSO_context_name_list() , p_context_definition_list() , user_information()  {}
		 ...
		 template<typename Archive> void serialize(Archive& arch){
			...
			BOOST_ASN_CHOICE(p_context_definition_list);
			BOOST_ASN_IMPLICIT_TAG(user_information ,30);
		}
	};
...
//It is   SEQUENCE  
	...
	struct  AARE_apdu__impl{
		...
		BOOST_ASN_VALUE_FUNC_DECLARATE(Mechanism_name ,  mechanism_name)
 		...
 		Authentication_value  responding_authentication_value;
		boost::shared_ptr<ASO_context_name_list > aSO_context_name_list;   //  OPTIONAL
		BOOST_ASN_VALUE_FUNC_DECLARATE(ASO_context_name_list ,  aSO_context_name_list)
		...
		AARE_apdu__impl()  : aSO_context_name() , result() , result_source_diagnostic() , responding_AP_title() , responding_AE_qualifier() , responding_authentication_value() , aSO_context_name_list() , p_context_result_list() , called_asoi_tag() , calling_asoi_tag() , user_information()  {}
		 ...
		 template<typename Archive> void serialize(Archive& arch){
			...
			BOOST_ASN_CHOICE(result_source_diagnostic);
			BOOST_ASN_CHOICE(responding_AP_title);
			BOOST_ASN_CHOICE(responding_AE_qualifier);
			...
			BOOST_ASN_CHOICE(responding_authentication_value);
	...
	struct  AARQ_apdu__impl{
		...
		AARQ_apdu__impl()  : aSO_context_name() , called_AP_title() , called_AE_qualifier() , calling_AP_title() , calling_AE_qualifier() , calling_authentication_value() , aSO_context_name_list() , p_context_definition_list() , called_asoi_tag() , calling_asoi_tag() , user_information()  {}
		 ...
		 template<typename Archive> void serialize(Archive& arch){
			...
			BOOST_ASN_IMPLICIT_TAG(protocol_version ,0);
			BOOST_ASN_EXPLICIT_TAG(aSO_context_name ,1);
			BOOST_ASN_CHOICE(called_AP_title);
			BOOST_ASN_CHOICE(called_AE_qualifier);
			BOOST_ASN_EXPLICIT_TAG(called_AP_invocation_identifier ,4);
			BOOST_ASN_EXPLICIT_TAG(called_AE_invocation_identifier ,5);
			BOOST_ASN_CHOICE(calling_AP_title);
			BOOST_ASN_CHOICE(calling_AE_qualifier);
			...
			BOOST_ASN_CHOICE(calling_authentication_value);
...
//It is   CHOICE
	...
	struct  ACSE_apdu : public BOOST_ASN_CHOICE_STRUCT(ACSE_apdu_enum) {
		...
		BOOST_ASN_VALUE_CHOICE(aarq, AARQ_apdu,  ACSE_apdu_AARQ_apdu)
		BOOST_ASN_VALUE_CHOICE(aare, AARE_apdu,  ACSE_apdu_AARE_apdu)
		BOOST_ASN_VALUE_CHOICE(rlrq, RLRQ_apdu,  ACSE_apdu_RLRQ_apdu)
		BOOST_ASN_VALUE_CHOICE(rlre, RLRE_apdu,  ACSE_apdu_RLRE_apdu)
		BOOST_ASN_VALUE_CHOICE(abrt, ABRT_apdu,  ACSE_apdu_ABRT_apdu)
		BOOST_ASN_VALUE_CHOICE(adt, A_DT_apdu,  ACSE_apdu_A_DT_apdu)
		BOOST_ASN_VALUE_CHOICE(acrq, ACRQ_apdu,  ACSE_apdu_ACRQ_apdu)
		BOOST_ASN_VALUE_CHOICE(acrp, ACRP_apdu,  ACSE_apdu_ACRP_apdu)
			...
			if (arch.__input__()){
				...
				switch(arch.test_class()){
					...
					case 0x80: {
						...
						;}
					default:{
						 if(BOOST_ASN_BIND_TAG(value<AARQ_apdu > (true , ACSE_apdu_AARQ_apdu))) return; else free();
					 if(BOOST_ASN_BIND_TAG(value<AARE_apdu > (true , ACSE_apdu_AARE_apdu))) return; else free();
					 if(BOOST_ASN_BIND_TAG(value<RLRQ_apdu > (true , ACSE_apdu_RLRQ_apdu))) return; else free();
					 if(BOOST_ASN_BIND_TAG(value<RLRE_apdu > (true , ACSE_apdu_RLRE_apdu))) return; else free();
					 if(BOOST_ASN_BIND_TAG(value<ABRT_apdu > (true , ACSE_apdu_ABRT_apdu))) return; else free();
					 if(BOOST_ASN_BIND_TAG(value<A_DT_apdu > (true , ACSE_apdu_A_DT_apdu))) return; else free();
					 if(BOOST_ASN_BIND_TAG(value<ACRQ_apdu > (true , ACSE_apdu_ACRQ_apdu))) return; else free();
					 if(BOOST_ASN_BIND_TAG(value<ACRP_apdu > (true , ACSE_apdu_ACRP_apdu))) return; else free();
					;}}}
			else {
				 switch(type()){
					case ACSE_apdu_AARQ_apdu: {BOOST_ASN_BIND_TAG(value<AARQ_apdu > (false , ACSE_apdu_AARQ_apdu)); break;}
					case ACSE_apdu_AARE_apdu: {BOOST_ASN_BIND_TAG(value<AARE_apdu > (false , ACSE_apdu_AARE_apdu)); break;}
					case ACSE_apdu_RLRQ_apdu: {BOOST_ASN_BIND_TAG(value<RLRQ_apdu > (false , ACSE_apdu_RLRQ_apdu)); break;}
					case ACSE_apdu_RLRE_apdu: {BOOST_ASN_BIND_TAG(value<RLRE_apdu > (false , ACSE_apdu_RLRE_apdu)); break;}
					case ACSE_apdu_ABRT_apdu: {BOOST_ASN_BIND_TAG(value<ABRT_apdu > (false , ACSE_apdu_ABRT_apdu)); break;}
					case ACSE_apdu_A_DT_apdu: {BOOST_ASN_BIND_TAG(value<A_DT_apdu > (false , ACSE_apdu_A_DT_apdu)); break;}
					case ACSE_apdu_ACRQ_apdu: {BOOST_ASN_BIND_TAG(value<ACRQ_apdu > (false , ACSE_apdu_ACRQ_apdu)); break;}
					case ACSE_apdu_ACRP_apdu: {BOOST_ASN_BIND_TAG(value<ACRP_apdu > (false , ACSE_apdu_ACRP_apdu)); break;}
					default:{}}}
		}
	};
