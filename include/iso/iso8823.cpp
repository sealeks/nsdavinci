...
namespace boost {
    namespace itu {
        ...
        namespace x226impl {
            ...
            static asncoder_ptr build_by_syntaxes(const oid_type& asyntax, const encoding_rule& tsyntax) {

                switch (tsyntax) {
                    case BER_ENCODING:
                    case DER_ENCODING:
                    case CER_ENCODING: return asncoder_ptr(new x690_archive(asyntax, tsyntax));
                    default:
                    {
                    }
                }
