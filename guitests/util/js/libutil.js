...
function init_project_controller(){
    ...
    if (dvnci_iseditable()) 
        libutil.designtime.getMainWindow();
}
...
libutil.designtime.getMainWindow = function (){
    libutil.www.create_tbwindow('maintool', 'Редактор' ,'100' , '100', '600','64','yes','yes',null,null,
        ...
        [function() {
            libutil.designtime.SaveAll();
        },
        function() {
            libutil.designtime.resetObjectInspector();
        },
        function() {
            libutil.designtime.resetFormInspector();
        },
        function() {
            libutil.designtime.resetLibInspector();
        },           
        function() {
            libutil.designtime.destroyMainWindow();
        }],
        libutil.designtime.destroyMainWindow);
    ...
    libutil.designtime.setMainWindowToolStatus();
    tmp.maintool.focus();
...
libutil.designtime.setMainWindowToolStatus = function (val){ 
    if ((val==1 || !val)) 
        libutil.www.set_tbwindow_btnstatus('maintool', null, 'save', libutil.designtime.isNeedSave() ?  'on' :  'disabled');
    if ((val==2 || !val)) 
        libutil.www.set_tbwindow_btnstatus('maintool', null, 'objinsp', libutil.designtime.getObjectInspector() ?  'off' :  'on');
    if ((val==3 || !val)) 
        libutil.www.set_tbwindow_btnstatus('maintool', null, 'forminsp', libutil.designtime.getFormInspector() ?  'off' :  'on');
    if ((val==4 || !val)) 
        libutil.www.set_tbwindow_btnstatus('maintool', null, 'libinsp', libutil.designtime.getLibInspector() ?  'off' :  'on');
}


libutil.designtime.destroyMainWindow = function(){
        var tmp=libutil.global.getGlobal();
        if (tmp && tmp.maintool)
            tmp.maintool=undefined;
        if (!libutil.designtime.isNeedSave()){          
            dvnci_exit();
            return;}
...
libutil.designtime.getObjectInspector = function (force){
    var tmp=libutil.global.getGlobal();
    ...
    if (tmp && !tmp.objectinspectorwin){
        ...
        objectinspectorwin.onunload=libutil.designtime.destroyObjectInspector;
        ...
        var current=libutil.designtime.getCurrentRedactor();
        if (current)
...
libutil.designtime.resetObjectInspector = function(){
    var vis =  libutil.designtime.getObjectInspector();
    var tmp=libutil.global.getGlobal();
    if (vis && tmp.objectinspectorwin)
        tmp.objectinspectorwin.close();
    else
        libutil.designtime.getObjectInspector(true);
    libutil.designtime.setMainWindowToolStatus(2);
        
} 

libutil.designtime.getObjectInspectorDocument = function(){
    var tmp = libutil.global.getGlobal();
...
libutil.designtime.getObjectInspectorTbody = function(){
    var tmp = libutil.global.getGlobal();
...
libutil.designtime.showObjectInspector = function(){
    var tmp = libutil.designtime.getObjectInspector();
    if (tmp)
        tmp.focus();
}
...
libutil.designtime.closeObjectInspector = function(){
    var tmp = libutil.designtime.getObjectInspector();
    if (tmp.objectinspectorwin)
        tmp.objectinspectorwin.close();
}

libutil.designtime.destroyObjectInspector = function(){
    var tmp=libutil.global.getGlobal();
    if (tmp && tmp.objectinspectorwin)
...
libutil.designtime.getFormInspector = function (force){
    var tmp=libutil.global.getGlobal();
    ...
    if (tmp && !tmp.formtool){
        ...
        libutil.www.create_tbwindow('formtool', 'Окна' ,'600','100', '600','200','yes','yes',null,null,
            ...
            [
            function() {
                libutil.designtime.addwindowfromfile();             
            },
            function() {
                libutil.designtime.addnewwindow();
            }]);
    
        var objdoc =tmp.formtool.document;
        tmp.formtool.onunload=libutil.designtime.destroyFormInspector;
    ...
    libutil.designtime.fillFormInspector();
    return (tmp && tmp.formtool) ? tmp.formtool : null;
}

libutil.designtime.resetFormInspector = function(){
    var vis =  libutil.designtime.getFormInspector();
    var tmp=libutil.global.getGlobal();
    if (vis && tmp.formtool)
        tmp.formtool.close();
    else
        libutil.designtime.getFormInspector(true);
    var tmp=libutil.global.getGlobal();
    tmp.formtool.focus();
    libutil.designtime.setMainWindowToolStatus(3);
        
} 

libutil.designtime.fillFormInspector = function (){
    var tmp=libutil.global.getGlobal();
    ...
    for (var i=0; i<fl.length; ++i ){
        ...
        libutil.designtime.fiCreateRow(tr,fl[i],'name');
        libutil.designtime.fiCreateRow(tr,fl[i],'caption');
        libutil.designtime.fiCreateRow(tr,fl[i],'left', '50px');       
        libutil.designtime.fiCreateRow(tr,fl[i],'top', '50px');
        libutil.designtime.fiCreateRow(tr,fl[i],'width', '50px');       
        libutil.designtime.fiCreateRow(tr,fl[i],'height', '50px'); 
        libutil.designtime.fiCreateRow(tr,fl[i],'visible', '50px'/*, ['','true','false']*/);
        libutil.designtime.fiCreateRow(tr,fl[i],'allwaystop', '50px'/*,, ['','true']*/);       
        libutil.designtime.fiCreateRow(tr,fl[i],'resizable', '50px'/*,, ['','true']*/);
        libutil.designtime.fiCreateRow(tr,fl[i],'decorated', '50px'/*,, ['','no']*/);
        libutil.designtime.fiCreateRow(tr,fl[i],'modal', '50px'/*,, ['','true']*/);        
        ...
        btno.setAttribute('onclick','libutil.designtime.resetwindow("'+formname+ '");');
        ...
        btnd.setAttribute('onclick','libutil.designtime.removeFormFromProject("'+formname+ '");');
    
    }
}

libutil.designtime.fiCreateRow = function(tr, tblrow, name, width, lst){
    ...
    td.onclick=function(ev) {
        libutil.designtime.fiPropertyRowFocus(ev);    
    }
}



libutil.designtime.fiPropertyRowFocus = function(ev){
    try{
        var td = ev.target;
        ...
        edit.onblur= function(ev) {
            libutil.designtime.fiPropertyLeaveFocus(ev);
        }
...
libutil.designtime.fiPropertyLeaveFocus = function(event){
    ...
    td.innerHTML=value;
    if ((oldval!=value) && (libutil.designtime.fiCheckFormParam(td.propname,value))){  
        ...
        if (td.elem && td.elem['element']){     
            ...
            libutil.designtime.closewindow(td.elem['name'], true);
            td.elem['param'] = libutil.project.buildparam(td.elem['element']);
            libutil.designtime.openwindow(td.elem['name']);
...
libutil.designtime.fiCheckFormParam = function(name, val){
        switch (name){
            case 'caption':{
                return true;
            }             
...
libutil.designtime.destroyFormInspector = function(){
    var tmp=libutil.global.getGlobal();
    if (tmp && tmp.formtool)
...
libutil.designtime.getLibInspector = function (force){    
    var tmp=libutil.global.getGlobal();
    ...
    if (!tmp.libtool){
        ...
        tmp.libtool.onunload=libutil.designtime.destroyLibInspector;
        ...
        for (var i=0; i<libs.length;++i){
            ...
            for (var j=0; j<comps.length;++j){
                ...
                btnsfunc.push(libutil.designtime.setSelectedToolEvent);
...
libutil.designtime.resetLibInspector = function(){
    
    var tmp=libutil.global.getGlobal();
    if (tmp.libtool)
        tmp.libtool.close();
    else
        libutil.designtime.getLibInspector(true);
    var tmp=libutil.global.getGlobal();
    tmp.libtool.focus();
    libutil.designtime.setMainWindowToolStatus(4);     
} 

libutil.designtime.setSelectedToolEvent = function(event){
    if (event && event.target && event.target.parentNode){
        var trgt=event.target.parentNode;
        libutil.designtime.setSelectedComponent(trgt.nametool, trgt.namebtn);
    }
}

libutil.designtime.setSelectedComponent = function(tool, comp){
    //alert(tool + ' : ' + comp);
    var tmp= libutil.global.getGlobal(); 
    var set  = tool && comp;
    var result = null; 
...
libutil.designtime.getSelectedComponent = function(){
    var tmp = libutil.designtime.setSelectedComponent();
    return (tmp && tmp.tool && tmp.component)? libutil.project.get_components(tmp.tool, tmp.component): null;
}


libutil.designtime.destroyLibInspector = function(){
    var tmp=libutil.global.getGlobal();
    if (tmp && tmp.libtool)
...
libutil.designtime.openwindow = function(name){
    var fl =  libutil.global.getFormList();
    if (fl){   
        for (var i=0; i<fl.length;++i){
            if (fl[i]['name']==name){
...
libutil.designtime.closewindow = function (name){
    var fl =  libutil.global.getFormList();
...
libutil.designtime.resetwindow = function (name){
    var fl =  libutil.global.getFormList(); 
    if (fl){  
        for (var i=0; i<fl.length;++i){
            try{
                if (fl[i]['name']==name){
                    if (fl[i].window)
                        libutil.designtime.closewindow(name);
                    else
                        libutil.designtime.openwindow(name);              
                }
            }
            catch(error){
                alert(error);
...
libutil.designtime.windowstatus = function (name){
    var fl =  libutil.global.getFormList(); 
    if (fl){  
        for (var i=0; i<fl.length;++i)
            if (fl[i]['name']==name)
...
libutil.designtime.SaveAll = function(){ 
    var fl= libutil.global.getFormList();
    for (var i=0; i<fl.length; ++i ){
        if (fl[i].red){
            fl[i].red.save();
...
libutil.designtime.isNeedSave = function (){
    var fl= libutil.global.getFormList();
    for (var i=0; i<fl.length; ++i ){
...
libutil.designtime.setCurrentRedactor = function (win){
    var tmp= libutil.global.getGlobal();
    if (!win){
        tmp.currentred = null;
        return;
...
libutil.designtime.getCurrentRedactor = function (){
    var tmp= libutil.global.getGlobal();
    return tmp.currentred;
}

libutil.designtime.findwindow = function (name){
    var fl =  libutil.global.getFormList();
    if (fl){  
        for (var i=0; i<fl.length;++i){
            if (fl[i]['name']==name){
...
libutil.designtime.windowexists = function (name){
    var fl =  libutil.global.getFormList();
    if (fl){  
        for (var i=0; i<fl.length;++i){
            if (fl[i]['name']==name){
...
libutil.designtime.addwindowfromfile = function (){
    ...
    if (libutil.designtime.windowexists(openform)){
        alert('Форма с именем '+openform+' уже существует!');
        return;
    }
    libutil.designtime.addform(openfile,openform); 
}
               
libutil.designtime.addnewwindow = function (){
    var openfile= prompt('Введите имя файла','');
    if (!openfile || openfile=='') return;          
    ...
    libutil.designtime.createfileform(openfile);
    libutil.designtime.addform(openfile,openform);
           
}

libutil.designtime.addform = function(file, name){
    try{       
        var doc = libutil.global.getStartupDoc();
        var elp=doc.getElementsByTagName('project')[0];           
        var els=doc.getElementsByTagName('form');
...
libutil.designtime.createfileform = function(file){
    var tmp=libutil.global.getGlobal();
    var prjpath=tmp.projectPath;
...
libutil.designtime.removeFormFromProject =function(name){
...
libutil.designtime.propertydialog = function(name, value){
